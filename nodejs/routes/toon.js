...
router.post('/episodelist', function(req, res){
  ...
  var sql = 'select toon_id, epi_name as curr_epi, epi_url, epi_thumb_url, epi_date, (select exists (select * from bookmark as b, epi_info as e where b.toon_id = e.toon_id and e.toon_id = ? and b.epi_name = curr_epi and b.user_id = ?)) as bm_flag from epi_info where toon_id = ? order by epi_date desc, epi_name*1 desc';
...
router.post('/firstepi', function(req, res){
  var toon_id = req.body.toon_id;

  var sql = 'select epi_url from epi_info where toon_id = ? order by epi_date asc, epi_name*1 asc limit 1';
